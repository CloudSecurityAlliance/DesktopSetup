...
- **Node is pinned to the LTS line on both platforms** (#56): `brew install node@24` on macOS, `winget install OpenJS.NodeJS.LTS` on Windows. macOS used to take the unversioned `node` formula, which is *Current* (26.x), so the platforms ran different majors for no stated reason. `CSA_NODE_MIN` / `$CsaNodeMin` is **22**, derived from Wrangler's `engines.node` — the consumer that asks for most — and is now enforced on **both**. Windows had no floor check at all: an existing non-winget Node of any age was accepted with "already installed", and Wrangler then failed at use time on a machine the installer had just called ready.

  **`node@24` is keg-only, and three things about that were measured on a `macos-latest` runner (2026-09-16) rather than reasoned about.** `brew install node@24` **exits 1 even when it succeeds** — it installs the formula, prints its Summary, then the keg-only caveat, and returns non-zero — so `install_node` checks whether the formula is installed rather than trusting the status; the first version of this aborted a perfectly good install. Keg-only is literal: immediately after that successful install there is **no `node` on PATH at all** until `brew link --overwrite --force node@24` runs, which is required, not defensive. And npm's global prefix is the Homebrew prefix in every state, so `wrangler`, `codex` and `gemini` installed with `npm -g` survive the switch — verified by installing wrangler first and confirming it still ran. The unversioned formula is **removed**, not merely unlinked, because `brew upgrade` in `macos-update.sh` would otherwise relink it and flip the machine back to Current between runs. The whole path was acceptance-tested on a real Mac from the state an existing CSA machine is in: node 26 plus a global wrangler going in, node 24 with a working wrangler coming out, idempotent on a second run.
...
4. **Output is silent unless registration actually happened.** On success, print a `Registered Claude Code MCP server: csa-mcp` line, then `Run /mcp inside Claude Code to authenticate with the CSA MCP server.` (the OAuth flow is browser-driven and must be initiated by the user), then a line naming what signing in needs: a **free** CSA account from `https://cloudsecurityalliance.org/`, via the site's `Sign in or Sign Up` control (label verified against the live page, 2026-09-15). The preflight plan carries the same second line under its `CSA MCP server` entry. Note these are two different gates and the wording keeps them apart: **registration** is gated on GitHub CSA-Internal access, while **authenticating** to the running server is what the CSA account is for. The server still answers unauthenticated callers, so the account is what unlocks member tiers rather than what makes the server work at all. On `add` failure, print a warn line with the captured stderr indented underneath, matching the marketplace-add error format.
...
**The plan must name what actually runs.** These scripts are "show the plan → confirm → act", so a plan line that describes something other than the step behind it is a correctness bug, not cosmetics — the user consented to the wrong thing. Three had drifted and were fixed together: Python said "install via Homebrew"/"install via winget" long after #60 made **uv** the provider and those the fallback; the macOS Node line said "installed (v26.8.1)" without mentioning that #74 now *removes* that unversioned formula and replaces it with `node@24`, hiding the only destructive step in the script; and the Windows plan never mentioned `uv` or the document preflight deps at all, though `Main` installed both. When you add a step to `main()`, add its plan line in the same change — `check-parity.py` compares main() steps across platforms but cannot tell whether the plan describes them.
